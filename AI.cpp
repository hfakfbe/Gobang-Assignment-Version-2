...
Gamenode* AI::Calculate(UNIT_SIZE layer, UNIT_SIZE maxlayer, UNIT_ID color) {//计算本节点的分数，更新父节点分数
	...
	delete[] v;
	if (gametree->head == nullptr) {//未落子状态，全盘均落子状态
	...
	while (true) {
		...
		if (tnode->calclayer != maxlayer - layer - 1) {/*疑似在1层以上死循环，应为maxlayer-layer-1*/
			//先转移到下一个node计算，再返回本node
			UNIT_SIZE x = tnode->x, y = tnode->y;
			chessboard[x][y] = color == PIECE_BLACK ? '1' : '2';
			gametree = tnode;/*可改为tnode*/
			...
			//删除
			if (t->father == nullptr) {
				gametree->head = t->son[d ^ 1];//t是gametree->head
			...
			/*head->son[1]就是t->father->son[1]为什么没有变成t->son[0]啊？*/
			/*t的father错误*/
			delete t;
			//插入
